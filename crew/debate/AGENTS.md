...
### This repository’s `debate` package (follow the code, not this blurb if they differ)

- Crew class: **`Debate`** in `src/debate/crew.py`, `@CrewBase`, configs `config/agents.yaml` and `config/tasks.yaml`, **`Process.sequential`**.
- **`main.py`** currently exports only **`run()`** (kickoff with `{"motion": ...}`). **`pyproject.toml` still lists `train`, `replay`, `test`, `run_with_trigger`**; those callables are missing from `main.py` unless you add them.
- **`tools/custom_tool.py`** is a template; agents in `crew.py` have **no tools**.
- **`knowledge/user_preference.txt`** is not loaded by the current crew code.

...
## This Project (engineering_team)

A crewAI project that turns plain-English software requirements into a working
Python module, a Gradio demo UI, and a unit-test suite. By default it builds a
trading-simulation account-management module (`accounts.py`).
...
│   │   ├── agents.yaml
│   │   └── tasks.yaml
│   ├── tools/custom_tool.py   # Stub BaseTool (not wired in)
│   ├── crew.py                # @CrewBase EngineeringTeam
│   └── main.py                # run() entry point
├── knowledge/user_preference.txt
├── output/                    # Generated artifacts
├── pyproject.toml             # Python >=3.10,<3.14; crewai[tools]==1.10.1; gradio
└── uv.lock
```

### Pipeline (Process.sequential)

| Step | Agent (in `agents.yaml`) | Task (in `tasks.yaml`) | Writes to |
|------|--------------------------|------------------------|-----------|
| 1    | `engineering_lead`       | `design_task`          | `output/{module_name}_design.md` |
| 2    | `backend_engineer`       | `code_task`            | `output/{module_name}` |
| 3    | `frontend_engineer`      | `frontend_task`        | `output/app.py` |
| 4    | `test_engineer`          | `test_task`            | `output/test_{module_name}` |

`code_task`, `frontend_task`, and `test_task` all declare
`context: [<previous task>]` so each stage receives the output of the previous
one.

### Agent specifics (from `crew.py`)

- All four agents pin `llm: gpt-4o-mini` in `agents.yaml`.
- `backend_engineer` and `test_engineer` set
  `allow_code_execution=True`, `code_execution_mode="safe"` (Docker sandbox),
  `max_execution_time=240`, `max_retry_limit=5`.
- `engineering_lead` and `frontend_engineer` are plain LLM agents with
  `verbose=True`.
- No tools are attached; the stub in `tools/custom_tool.py` is not registered.

### Inputs (from `main.py`)

`run()` calls `EngineeringTeam().crew().kickoff(inputs=...)` with:

- `requirements` — multi-line string describing the trading-account system.
- `module_name` — `"accounts.py"`.
- `class_name` — `"Account"`.

These names are interpolated into both YAML files via `{requirements}`,
`{module_name}`, `{class_name}` placeholders.

### How to run

```bash
crewai install     # uses pyproject.toml + uv.lock
crewai run         # entry: engineering_team.main:run

# Generated app + tests after a successful run:
cd output
python app.py
python -m unittest test_accounts.py
```

`OPENAI_API_KEY` must be set in `.env`. Docker must be available for the two
code-executing agents.

### When editing this project

- Adding/renaming an agent: update `agents.yaml`, then add a matching
  `@agent`-decorated method in `crew.py` (method name must match the YAML
  key).
- Adding/renaming a task: update `tasks.yaml`, add a matching
  `@task`-decorated method in `crew.py`, and remember to add `context: [...]`
  in YAML if it depends on an earlier task's output.
- Always keep `# type: ignore[index]` on `self.agents_config[...]` /
  `self.tasks_config[...]` accesses.
- `[project.scripts]` in `pyproject.toml` lists `train`, `replay`, `test`,
  `run_with_trigger` — these are reserved entries; only `run` /
  `engineering_team` / `run_crew` are implemented in `main.py`.

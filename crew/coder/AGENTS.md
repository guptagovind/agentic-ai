...
## This codebase: `coder` (`crew/coder/`)

When you edit **this** repository, treat the table below as source of truth alongside `pyproject.toml` and `src/coder/`. The rest of this file is **general** CrewAI guidance.

| Item | In this project |
|------|------------------|
| **Directory** | Project root = folder containing `pyproject.toml` (the `coder` package). |
| **CrewAI version** | `crewai[tools]==1.10.1` (pinned in `pyproject.toml`). |
| **Python** | `>=3.10,<3.14`. |
| **Crew class** | `Coder` in `src/coder/crew.py` with `@CrewBase`. |
| **Process** | `Process.sequential`. |
| **Agent YAML key** | `coder` — role *Python Developer*, `llm: gpt-4o-mini`. |
| **Task YAML key** | `coding_task` — `output_file: output/code_and_output.txt`, description uses `{assignment}`. |
| **Kickoff inputs** | `main.py` passes `{"assignment": "<string>"}`. |
| **Code execution** | `allow_code_execution=True`, `code_execution_mode="safe"` → **Docker** must be installed and usable (`docker info` as the run user). |
| **Agent limits** | `max_execution_time=120`, `max_retry_limit=3`. |
| **Custom tools** | `src/coder/tools/custom_tool.py` defines `MyCustomTool` (stub); **not** attached in `crew.py` unless you add it. |
| **Entry points** | `run()` in `src/coder/main.py`. Use `crewai run` or `uv run run_crew` / `uv run coder`. `[project.scripts]` also lists `train`, `replay`, `test`, `run_with_trigger` — those callables are **not** defined in `main.py` yet. |
| **Generated output** | Task writes to `output/code_and_output.txt`; `output/` is gitignored; `main.py` creates `output/` if missing. |
...
### Crew Project
```
coder/                         # This repo’s package name (see pyproject.toml)
├── src/coder/
│   ├── config/
│   │   ├── agents.yaml       # Agent definitions (role, goal, backstory, llm)
│   │   └── tasks.yaml        # Task definitions (description, expected_output, agent, output_file)
│   ├── tools/
│   │   └── custom_tool.py    # Custom tool implementations (optional)
│   ├── crew.py               # @CrewBase class Coder — agents, tasks, Crew
│   └── main.py               # run(); kickoff(inputs=...); creates output/
├── knowledge/                 # Optional knowledge base resources
├── output/                    # Created at runtime; task output_file path lives here (gitignored)
